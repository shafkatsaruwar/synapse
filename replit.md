...
- 2026-02-24: Searchable Conditions system: `constants/conditions.json` (~50 conditions), `HealthCondition` interface + `conditionStorage` in storage, dedicated `HealthProfileConditionsScreen` with search/add/edit/delete/stress-dose-prompt; Settings shows "Health Profile" nav row with condition count; Dashboard/Reports/Insights read from conditionStorage; Onboarding reduced from 12 to 11 steps (conditions step removed)
- 2026-02-23: Added first-launch onboarding flow (11 screens): Welcome, Name input, Health Setup (medications), 6-screen founder story with progress dots. Saves onboardingCompleted flag to skip on subsequent launches
